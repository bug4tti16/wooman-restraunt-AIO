...
print (df)
for i in df.index:
    name=df['이름'][i].encode('utf-8').decode('utf-8')
    rfid=df['카드번호'][i].encode('utf-8').decode('utf-8')
    #print (name,rfid)
    cur.execute(f"""
...
for i in df.index:
    ...
    for x in col:
        if type(df[x][i])==list:
            ...
            d=df[x][i][0] + dt.timedelta(0,30)
            date2=d.isoformat(sep=' ',timespec="seconds")
            #print (date)
            if df[x][i][2]:
                cur.execute(f"""
                INSERT INTO  log (UID, Time, Cbool)
                VALUES ('{num}', '{date}', 0)""")
            if df[x][i][2]==False:
                cur.execute(f"""
                INSERT INTO  log (UID, Time, Cbool)
            ...
            if '죽' in df[x][i][1]:
                cur.execute(f"""
                INSERT INTO  log (UID, Time, Menu)
                VALUES ('{num}', '{date2}', 1)""")
            if '죽' not in df[x][i][1]:
                cur.execute(f"""
                INSERT INTO  log (UID, Time, Menu)
                VALUES ('{num}', '{date2}', 0)""")
